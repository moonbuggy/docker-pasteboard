...
nginx="$(wget -qO- http://localhost:8080/nginx-ping)"
status="$(wget -qO- localhost:3000/status | sed -En 's|.*message\W+(\w*).*|\1\n|p')"
proxied_status="$(wget -qO- localhost:8080/status | sed -En 's|.*message\W+(\w*).*|\1\n|p')"
